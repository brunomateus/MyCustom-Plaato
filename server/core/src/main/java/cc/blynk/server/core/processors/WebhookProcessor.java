...
import cc.blynk.server.core.plugins.PlaatoJson;
...
public class WebhookProcessor extends NotificationBase {
    ...
    public void processPlaato(User user, DashBoard dash, Device device, short pin, PinType pinType) {
        DashPinStorageKey dashPinStorageKey = new DashPinStorageKey(dash.id, device.id, pinType, pin);
        PinStorageValue pinStorageValue = user.profile.pinsStorage.get(dashPinStorageKey);
        if (!(pinStorageValue instanceof SinglePinStorageValue)) {
            return;
        }

        String url = ((SinglePinStorageValue) pinStorageValue).value;

        log.trace("Start ProcessPlaato webhook... Url {}", url);
        if (!WebHook.isValidUrl(url) || device.plaato == null) {
            return;
        }

        BoundRequestBuilder builder;
        try {
            builder = httpclient.prepare(SupportedWebhookMethod.POST.name(), url);
        ...
        PlaatoJson body = new PlaatoJson(device.id, device.getNameOrDefault(), device.plaato);
        builder.setBody(body.toString());

        log.trace("ProcessPlaato webhook sent.");
        builder.execute(new AsyncCompletionHandler<Response>() {
            @Override
            public Response onCompleted(Response response) {
                if (isValidResponseCode(response.getStatusCode())) {
                    if (response.hasResponseBody()) {
                        log.debug("Sending webhook succeeded");
                    }
                } else {
